...
void Channel::handleevent()
{
    ...
    {
        if (closecallback_) closecallback_();      // 回调Connection::closecallback()。
    }

    if (revents_ & (EPOLLIN|EPOLLPRI))   // 接收缓冲区中有数据可以读。
    {
        if (readcallback_) readcallback_();   // 如果是acceptchannel，将回调Acceptor::newconnection()，如果是clientchannel，将回调Connection::onmessage()。
    }

    if (revents_ & EPOLLOUT)                  // 有数据需要写。
    {
        if (writecallback_) writecallback_();      // 回调Connection::writecallback()。     
    }
}
