...
void Connection::send(const char *data,size_t size)        
{
    ...
    else
    {
        ...
        // 注意：data 指向的内存可能来自调用线程的临时对象，不能直接传入队列。
        // 这里复制数据到堆上（使用 shared_ptr<string>）并在 lambda 中传递，
        // 以保证数据在 IO 线程处理前不会被释放。
        auto msg = std::make_shared<std::string>(data, size);
        // 捕获 shared_ptr<Self>，确保 Connection 在 lambda 执行前不会被析构，避免 use-after-free。
        auto self = shared_from_this();
        loop_->queueinloop([self, msg]() { self->sendinloop(msg->data(), msg->size()); });
    }
}
