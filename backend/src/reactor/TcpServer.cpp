...
{
    // 把新建的conn分配给从事件循环。
    // 注意：避免在同一表达式中既使用又移动 `clientsock`，这会受限于参数求值顺序，
    // 在某些编译器/标准下可能导致对空指针的访问。
    if (!clientsock) return;
    int cfd = clientsock->fd();
    if (cfd < 0) { printf("accept returned invalid fd=%d\n", cfd); return; }
    ...
    int idx2 = conn->fd() % threadnum_;
    if (idx2 < 0) idx2 += threadnum_;
    subloops_[idx2]->newconnection(conn);       // 把conn存放到EventLoop的map容器中。
